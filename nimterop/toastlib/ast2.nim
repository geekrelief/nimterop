...
proc addEnum(gState: State, node: TSNode) =
  ...
  if not enumlist.isNil:
    ...
    if name.Bl:
      ...
      let defineNode = gState.parseString(&"defineEnum({name}, {gState.enumNotDistinct}, \"{gState.enumBaseTypeStr}\")")
      # nkStmtList(
      #  nkCall(
      #   nkIdent("defineEnum"),
        ...
        let
          fval = block:
            ...
            if origname.nBl:
              ...
              let defaultEnumType = "." & gState.enumBaseTypeStr
              if not pnode.isnil:
                case pnode.kind:
                  of nkInt64Lit:
                    fval &= ".clonglong"
